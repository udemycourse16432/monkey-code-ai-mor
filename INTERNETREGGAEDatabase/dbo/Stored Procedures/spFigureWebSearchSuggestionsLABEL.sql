CREATE PROCEDURE [dbo].[spFigureWebSearchSuggestionsLABEL]
  @Hint nvarchar(255),
  @Word1 nvarchar(50),
  @Word2 nvarchar(50),
  @Word3 nvarchar(50),
...
declare @QtyOther int
  ...
  if @QtyCD+@QtyVinyl+@QtyOther>0
   begin
    if not exists(select counter from WebSearchSuggestions where Hint=@Hint and SearchType='Label')
     begin
     ...
     begin
      ...
      ,Vinyl=@QtyVinyl
      ,Other=@QtyOther
      where Hint=@Hint and SearchType='Label'
     end
    end
  else
   delete WebSearchSuggestions where Hint=@Hint and SearchType='Label'
