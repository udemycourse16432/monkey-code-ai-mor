...
-- =============================================
CREATE PROCEDURE [dbo].[spFigureCustomerCartNotificationsErnie]
...
select CustomerServerCounter from SignInCartTotalsTableErnie
...
select (select count(CustomerServerCounter) from SignInCartTotalsTableErnie
where TotalPrice>=@MinimumCartDollarAmount
and (datediff(day,LastCartAdjustment,getdate())>=@CartStaleForHowManyDays or LastCartAdjustment is null)
